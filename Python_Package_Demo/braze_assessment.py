...
class DataModeler:
    def __init__(self, sample_df: pd.DataFrame):
        ...
        self.train_df = sample_df.drop(['outcome'], axis=1)
        self.train_df_out = sample_df['outcome']
        self.model = DecisionTreeClassifier(random_state=3)
    ...
    def prepare_data(self, oos_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        # ** Your code here **
        if oos_df is None:
            self.train_df = self.train_df.drop(['customer_id'], axis=1)
            self.train_df['transaction_date'] = pd.to_datetime(self.train_df['transaction_date'])
            self.train_df['transaction_date'] = self.train_df['transaction_date'].values.astype('float64')
            self.train_df['transaction_date'] = self.train_df['transaction_date'].where(self.train_df['transaction_date'] > 0)
        else:
            prepared_df = oos_df.drop(['customer_id'], axis=1)
            prepared_df['transaction_date'] = pd.to_datetime(prepared_df['transaction_date'])
            prepared_df['transaction_date'] = prepared_df['transaction_date'].values.astype('float64')
            prepared_df['transaction_date'] = prepared_df['transaction_date'].where(prepared_df['transaction_date'] > 0)
            return prepared_df
    ...
    def impute_missing(self, oos_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        if oos_df is None:
            ...
            return oos_df.fillna(oos_df.mean())
    ...
    def fit(self) -> None:
        ...
        '''
        # ** Your code here **
        self.model.fit(self.train_df, self.train_df_out)
    ...
    def model_summary(self) -> str:
        ...
        # ** Your code here **
        model_params_dict = {}
        model_params_dict['model_name'] = self.model.__class__.__name__
        model_params_dict['model_depth'] = int(self.model.get_depth())
        model_params_dict['model_n_leaves'] = int(self.model.get_n_leaves())
        model_params_dict['model_params'] = self.model.get_params()
